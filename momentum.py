...
def cap_signal(before,after):
    ...
    if before['contract']!=after['contract'] or not a.get('pool') or a['pool']!=b.get('pool') or not 0<elapsed<=10:return None
    ...
    return {'id':f'cap:{direction}:{after["contract"]}:{after["observed_at"]}','contract':after['contract'],
        ...
        'before_id':before['id'],'after_id':after['id'],'elapsed_seconds':elapsed,'cap_before':a['market_cap'],'cap_after':b['market_cap'],
...
def signal(before,after):
    ...
    return {'id':'momentum:'+after['contract'], 'contract':after['contract'],'symbol':after.get('symbol','Unknown'),
        ...
        'before_id':before['id'],'after_id':after['id'],'elapsed_seconds':elapsed,
...
def view(con,now):
    ...
    return {'alerts':[{**a,'paper':({'status':'Downward alert only; no paper entry','outlay':None} if a.get('direction')=='fall' else paper_update(a,samples,now))} for a in alerts],
            'tracking':tracking,'status':f'Market-cap alerts active · {len(tracking)} tokens with recent cap observations',
            'rule':'Within 10 seconds, same mint/pool: rise ≥50% or fall ≥30%, with at least $2,000 absolute change. Market polling targets every 5 seconds; provider updates may lag.',
            'coverage':'The combined $3,500→$10,000 and 2→15-holder rule remains separate and awaits complete holder data. The bounded collector does not monitor every launch; unindexed tokens lack quotes.'}

def ingest(con,current):
    import json
    from app import put
    previous=[json.loads(r[0]) for r in con.execute("SELECT payload FROM records WHERE type IN ('screening','fast_market') AND observed_at>=? ORDER BY observed_at DESC LIMIT 2000",(datetime.fromtimestamp(ts(current['observed_at'])-60,__import__('datetime').timezone.utc).isoformat(),))]
    # Oldest valid baseline within the window; do not invent a full 60s history.
    found=[]
    for detector in (signal,cap_signal):
        for older in reversed(previous):
            alert=detector(older,current)
            if not alert:continue
            if alert.get('kind')=='cap_only':
