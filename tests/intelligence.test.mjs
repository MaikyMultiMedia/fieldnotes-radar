...
import {leaderboard,normalizeLeaders} from '../server/intelligence.mjs';
const wallet='3'.repeat(32);
const row=(pnl=120)=>({owner:wallet,total_value:100001,trade_count:10,volume_usd:'15000',realized_pnl:pnl,unrealized_pnl:null});
const request=w=>new Request('https://workspace.test/api/intelligence/leaders?window='+w);
function database(){const db=new DatabaseSync(':memory:');for(const f of fs.readdirSync('drizzle').filter(f=>f.endsWith('.sql')).sort())db.exec(fs.readFileSync('drizzle/'+f,'utf8'));
