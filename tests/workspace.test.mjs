...
test('server enforces authentication, origin, sharing, revisions and logout',async()=>{
  ...
  for(const path of ['/api/alerts/scan','/api/alerts/rules','/api/alerts/seen/'+('a'.repeat(64)), '/api/paper','/api/paper/observe','/api/paper/'+crypto.randomUUID()+'/close']){
...
test('GitHub release switches atomically only after all hashes verify',async()=>{
  ...
  globalThis.fetch=async url=>new Response(String(url).endsWith('workspace-release.json')?JSON.stringify({apiVersion:6,commit,assets}):files[String(url).split('/').pop()]);
  ...
  globalThis.fetch=async url=>new Response(String(url).endsWith('workspace-release.json')?JSON.stringify({apiVersion:6,commit,assets}):'tampered');
