...
test('server enforces authentication, origin, sharing, revisions and logout',async()=>{
  assert.equal((await worker.fetch(req('/api/workspace'),env)).status,401);
...
test('GitHub release switches atomically only after all hashes verify',async()=>{
  ...
  globalThis.fetch=async url=>new Response(String(url).endsWith('workspace-release.json')?JSON.stringify({apiVersion:1,commit,assets}):files[String(url).split('/').pop()]);
  ...
  globalThis.fetch=async url=>new Response(String(url).endsWith('workspace-release.json')?JSON.stringify({apiVersion:1,commit,assets}):'tampered');
