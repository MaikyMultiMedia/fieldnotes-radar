...
import {savedBuyEvidence} from './buy-alerts.mjs';
...
export function assumptions(input){
  ...
  if(input.holdSeconds!==undefined){const hold=number(input.holdSeconds);if(![0,300,900,3600].includes(hold))throw fail('Choose a manual exit or a 5, 15 or 60 minute paper exit.');if(hold)value.holdSeconds=hold;}
  return value;
}
...
export function advancePaper(trial,q,now){
  ...
  if(t.status==='open'&&t.exitDeadlineAt&&now>t.exitDeadlineAt){t.status='missed_exit';t.endedAt=iso(now);t.lastCheck={at:iso(now),outcome:'missed_exit',reason:'No usable exit snapshot was captured inside the planned five-minute window. No closing P&L is assigned.'};return t;}
  ...
  if(t.status==='waiting'){
    ...
    if(t.assumptions.holdSeconds){t.exitDueAt=fetched+t.assumptions.holdSeconds*1000;t.exitDeadlineAt=t.exitDueAt+300000;}
  }
  ...
  t.lastCheck={at:iso(now),outcome:'observed',reason:'Provider snapshot captured; no execution quote or fill verified.'};
  if(t.exitDueAt&&fetched>=t.exitDueAt&&fetched<=t.exitDeadlineAt){t.status='closed';t.exit={...mark,closedAt:iso(now),closedBy:null,method:'planned',targetAt:iso(t.exitDueAt),delaySeconds:(fetched-t.exitDueAt)/1000,holdingSeconds:(fetched-Date.parse(t.entry.at))/1000};}
  return t;
}
export function paperView(t,now){
  const status=t.status==='waiting'&&now>t.deadlineAt?'missed':t.status==='open'&&t.exitDeadlineAt&&now>t.exitDeadlineAt?'missed_exit':t.status;
...
export function paperSummary(trials){
  ...
  const result={total:trials.length,waiting:trials.filter(t=>t.status==='waiting').length,open:trials.filter(t=>t.status==='open').length,closed:closed.length,missed:trials.filter(t=>['missed','missed_exit'].includes(t.status)).length,cancelled:trials.filter(t=>t.status==='cancelled').length,closedPnl:closed.length?closed.reduce((sum,t)=>sum+t.exit.pnl,0):null,winRate:closed.length?closed.filter(t=>t.exit.pnl>0).length/closed.length*100:null};
  result.byTag=PAPER_TAGS.map(tag=>{const all=trials.filter(t=>t.tag===tag),done=closed.filter(t=>t.tag===tag);return{tag,total:all.length,closed:done.length,missed:all.filter(t=>['missed','missed_exit'].includes(t.status)).length,cancelled:all.filter(t=>t.status==='cancelled').length,meanReturnPct:done.length?done.reduce((sum,t)=>sum+t.exit.returnPct,0)/done.length:null,pnl:done.length?done.reduce((sum,t)=>sum+t.exit.pnl,0):null};});
  result.byPlan=[0,300,900,3600].map(holdSeconds=>{
    const all=trials.filter(t=>(t.assumptions?.holdSeconds||0)===holdSeconds),done=all.filter(t=>t.status==='closed'&&finite(t.exit?.returnPct)),returns=done.map(t=>t.exit.returnPct).sort((a,b)=>a-b),mid=Math.floor(returns.length/2);
    return {holdSeconds,total:all.length,closed:done.length,active:all.filter(t=>['waiting','open'].includes(t.status)).length,missedEntry:all.filter(t=>t.status==='missed').length,missedExit:all.filter(t=>t.status==='missed_exit').length,cancelled:all.filter(t=>t.status==='cancelled').length,meanReturnPct:done.length?returns.reduce((a,b)=>a+b,0)/done.length:null,medianReturnPct:done.length?(returns.length%2?returns[mid]:(returns[mid-1]+returns[mid])/2):null};
  });return result;
}
...
export async function paper(request,env,user,input=null){
  ...
  if(path==='/api/paper'&&request.method==='POST'){
    ...
    if(prior){if(prior.chain!==chain||prior.contract!==contract||prior.pool!==pool||prior.tag!==tag||(prior.sourceSignal?.id||null)!==(input.signalId||null)||(prior.sourceBuy?.id||null)!==(input.buyAlertId||null)||prior.thesis!==thesis||JSON.stringify(prior.assumptions)!==JSON.stringify(a))throw fail('This request ID already belongs to another trial.',409);return {trial:paperView(prior,now)};}
    ...
    if(input.buyAlertId&&tag!=='large_buy')throw fail('Use the large-buy reason for linked buy evidence.');
    const sourceBuy=input.buyAlertId?await savedBuyEvidence(env,input.buyAlertId,{chain,contract,pool}):null;
    const count=await env.DB.prepare("SELECT count(*) AS total,sum(CASE WHEN (status='open' AND (json_extract(payload,'$.exitDeadlineAt') IS NULL OR json_extract(payload,'$.exitDeadlineAt')>=?)) OR (status='waiting' AND eligible+300000>=?) THEN 1 ELSE 0 END) AS active FROM paper_trials").bind(now,now).first();
    ...
    t.sourceSignal=sourceSignal;t.sourceBuy=sourceBuy;
  ...
  if(path==='/api/paper/observe'&&request.method==='POST'){
    ...
    const exits=await env.DB.prepare("UPDATE paper_trials SET status='missed_exit',updated=?,revision=revision+1,payload=json_set(payload,'$.status','missed_exit','$.endedAt',?,'$.lastCheck',json(?)) WHERE status='open' AND json_extract(payload,'$.exitDeadlineAt')<?").bind(now,iso(now),JSON.stringify({at:iso(now),outcome:'missed_exit',reason:'No usable exit snapshot was captured inside the planned five-minute window. No closing P&L is assigned.'}),now).run();
    if(exits.meta.changes)await audit(env,user,'recorded missed paper exits',String(exits.meta.changes),now);
    const rows=await env.DB.prepare("SELECT payload,revision FROM paper_trials WHERE status='open' OR (status='waiting' AND eligible<=?) ORDER BY CASE WHEN status='open' AND json_extract(payload,'$.exitDueAt')<=? THEN 0 WHEN status='waiting' THEN 1 ELSE 2 END,updated ASC LIMIT 4").bind(now,now).run();
    const results=[];
    for(const row of rows.results){const before=decode(row);try{
      ...
      const after=advancePaper(before,q,Date.now());if(error&&!['missed','missed_exit'].includes(after.status))after.lastCheck.reason=error;
      const saved=await update(env,before,after,Date.now());
      if(before.status!==after.status)await audit(env,user,after.status==='open'?'observed paper entry':after.status==='closed'?'recorded planned paper exit':after.status==='missed_exit'?'missed paper exit':'missed paper entry',after.symbol,Date.now());
    ...
    return {results,missedEntries:expired.meta.changes,missedExits:exits.meta.changes};
  }
  ...
  if(match&&request.method==='POST'){
    ...
    if(action==='reflection'){
      ...
      after.status=paperView(t,now).status==='missed'?'missed':paperView(t,now).status==='missed_exit'?'missed_exit':'cancelled';after.endedAt=iso(now);after.endReason=reason;
    }else{
      if(t.status!=='open')throw fail('A paper entry must be observed before it can close.',409);
      if(t.assumptions.holdSeconds)throw fail('This trial has a fixed exit plan. Capture observations to record it, or cancel with a reason.',409);
      const q=await quote(request,env,t),problem=quoteProblem(q,t,Date.now());if(problem)throw fail(problem,422);
      ...
      after.status='closed';after.exit={...after.lastMark,closedAt:iso(Date.now()),closedBy:user,method:'manual'};
    }
