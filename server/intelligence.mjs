...
export async function leaderboard(request,env){
  ...
  if(old&&now-old.fetched<300)return result(JSON.parse(old.payload),old.fetched,'fresh');
  ...
  const task=(async()=>{
    try{
      ...
      const response=await fetch(ROOT+'/wallet/v2/leaderboard?'+params,{headers:{'X-API-KEY':env.BIRDEYE_API_KEY,'x-chain':'solana',Accept:'application/json'},signal:AbortSignal.timeout(10000)});
      if(response.status===429){await env.DB.prepare("UPDATE market_budget SET blocked_until=? WHERE id='birdeye'").bind(now+Math.min(3600,Math.max(60,Number(response.headers.get('retry-after'))||60))).run();throw fail('Birdeye is rate-limiting rankings. Try again shortly.',429);}
    ...
    }catch(e){if(old&&now-old.fetched<=3600)return result(JSON.parse(old.payload),old.fetched,'stale',e.status?e.message:'Refresh failed.');throw e.status?e:fail('Profit data could not be refreshed.');}
