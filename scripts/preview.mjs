...
const env={GMGN_API_KEY:process.env.GMGN_API_KEY,BIRDEYE_API_KEY:process.env.BIRDEYE_API_KEY,DB:{prepare:prepared,async batch(list){db.exec('BEGIN');try{const results=[];for(const p of list)results.push(await p.run());db.exec('COMMIT');return results;}catch(e){db.exec('ROLLBACK');throw e;}}},ACCOUNT_CREDENTIALS:fs.readFileSync('.local/accounts.json','utf8')};
