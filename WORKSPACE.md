...
The frontend contract uses API version 11. Changes to API contracts must keep compatible interface behavior or intentionally change the release manifest version and deploy a matching Worker. The Worker never evaluates remotely fetched backend code.
...
Market freshness is based on a valid past receipt time and an explicit fresh status. Future-dated cache entries are neither current data nor outage fallbacks. Chart candles and pool details retain separate receipts; cached candles cannot restore older pool details after a newer pool response. Large-buy rows require both receipts to be current, and their coverage panel labels the trade feed and pool details separately. Research opened from a trade retains the pool receipt. A recent API response does not establish a recent exchange observation.

GeckoTerminal, Birdeye and GMGN reads reject redirects so fixed provider origins and optional API credentials remain bounded to the configured destination. GeckoTerminal and Birdeye cooldowns accept both numeric and HTTP-date Retry-After values, bounded to one minute through one hour. Existing no-key behavior, request budgets and API contract 11 remain unchanged. Synthetic regression tests cover clock skew, mixed chart/pool cache ages, unavailable refreshes and cooldowns.
...
## Planned paper exits and linked buy evidence

New trials can precommit to a 5, 15 or 60 minute holding target, or retain manual closure. The form defaults to 15 minutes; existing trials without a plan remain manual. The immutable target starts from the actual later entry snapshot, not the signal's time or the initial button click. A planned close uses the first fresh, exact-pool snapshot with receipt time at/after that target and captured no later than five minutes afterward. Existing size/liquidity and assumed-cost checks still apply. The record retains target, capture deadline, actual holding time and exit delay. It neither checks a Raydium route automatically nor assumes a Fomo execution.

A missing entry remains `missed`; a missing planned exit becomes `missed_exit`, with no closed PnL and no historical candle replacement. Reads expose overdue records immediately; observation requests persist expired statuses without upstream calls for those records. Missed exits do not consume the active-trial quota. Manual close is refused for a planned trial. Cancellation is still possible with a recorded reason and remains in the comparison denominator; cancellation after the deadline retains the missed status. Repeated observations cannot rewrite a closed result, and revision checks protect concurrent writes.

The existing visible-journal collector still samples at most four due trials per minute, within the shared free provider budget. Due planned exits take priority, followed by waiting entries and ordinary open marks. Closing, hiding or leaving the journal can miss a window. Browser timer throttling, busy cohorts, free-provider limits and unavailable snapshots can also cause misses. No background scheduler, always-on service or trade automation is added.

Buy alerts now offer Paper-test this buy. The server loads the retained alert by ID, checks network/contract/pool and buy type, and embeds its evidence in the private paper record. Client-supplied evidence cannot replace it. The original transaction, source, block/detection/receipt times, rule and sample coverage survive later alert expiry. A new trial obtains a current starting snapshot and the usual delayed entry; it never backdates a fill to the observed buyer. Request idempotency checks include the linked alert and planned horizon.

Separate exit-plan groups show all saved trials, closed and active counts, missed entries/exits, cancellations, and median/mean closed returns. All statuses stay in the group denominator, but missing/cancelled trials have no return. The sample remains selected by users, with differing tokens, entry delays, assumed costs and up to five minutes of exit latency. These are descriptive paper observations, not a validated strategy, complete alert cohort, wallet profit ranking or proof of obtainable returns. API compatibility contract 11 protects older servers. The existing JSON payload storage accommodates the new optional fields and status; no schema migration or new provider is needed.
